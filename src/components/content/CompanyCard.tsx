...
export function CompanyCard({ name, slug, description, sector, accentColor, lang }: CompanyCardProps) {
  return (
    <Link href={`/companies/${slug}`} className="block h-full group relative">
      {/* Removed Hover Glow Effect as requested */}
      <Card className="h-full p-8 border border-black/5 dark:border-white/10 hover:border-black/10 dark:hover:border-white/20 transition-all duration-500 relative overflow-hidden bg-white dark:bg-white/5 backdrop-blur-xl rounded-2xl shadow-xl dark:shadow-2xl group-hover:-translate-y-2">
        ...
        <div className="flex flex-col h-full gap-6 relative z-10">
          ...
          <div className="pt-6 flex items-center justify-between border-t border-black/10 dark:border-white/10 mt-auto">
            <span className="text-sm font-medium text-black/70 dark:text-white/70 group-hover:text-black dark:group-hover:text-white transition-colors tracking-wide uppercase">
              {lang === "tr" ? "Detaylı İncele" : "View Details"}
            </span>
            <div className="w-10 h-10 rounded-full border border-black/20 dark:border-white/20 flex items-center justify-center text-black/70 dark:text-white/70 group-hover:bg-accent group-hover:text-white dark:group-hover:text-primary group-hover:border-accent transition-all duration-500 shadow-lg group-hover:shadow-accent/50 group-hover:scale-110">
